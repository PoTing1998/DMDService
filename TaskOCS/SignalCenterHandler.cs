...
public class SignalCenterHandler
{
    ...
    private void InitializeModbusClient(int stationId, string ipAddress, int port)
    {
        int retryCount = 3;
        while (retryCount > 0)
        {
            try
            {
                ...
                Console.WriteLine($"Connected to Station {stationId}.");
                return;
            }
            catch (Exception ex) 
            {
                retryCount--;
                Console.WriteLine($"Retrying connection to Station {stationId}. Attempts left: {retryCount}. Error: {ex.Message}");
            }
        }

        throw new Exception($"Failed to connect to Station {stationId} after 3 attempts.");
    }
