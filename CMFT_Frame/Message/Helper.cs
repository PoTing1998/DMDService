    ...
    public class Helper
    {
        ...
        public static byte[] Pack(Message CMFTmessage)
        {
            ...
            try
            {
                ...
                CMFTmessage.CompleteContent = arrReturn;
        ...
        public static Message UnPack(byte[] msgBytes)
        {
            ...
            try
            {
                ...
                //若為heartbeat 則不須繼續解譯
                if (oReturn.MessageType == Message.eMessageType.Heartbeat)
                {
                    return oReturn;
                }
        ...
        /// <param name="jsonData">Json資料內容</param>
        /// <returns></returns>
        public static Object GetJsonObjectFromCAD(string jsonData)
        {
            if (jsonData != null && jsonData != "")
            {
                string sType = ASI.Lib.Text.Parsing.Json.GetValue(jsonData, "Type");
                string sJsonObjectName = $"ASI.Wanda.CMFT.JsonObject.CADServer.FromTETRA.{sType}";
                if (sType =="")
                {
                    sJsonObjectName = $"ASI.Wanda.CMFT.JsonObject.CADServer.FromTETRA.Response";
                }
